---
title: "Differential analysis with DESeq2 with no ampicillin treatment"
output: html_notebook
---

Authors: Huijing Wang and Christian Ray

- In this analysis, /*all gene statistics*/ are saved to .csv files for facilitating further analysis. 
- The normalized counts were also saved. 
- The adjusted p value (FDR) threshold is set to 0.05 for identifying significant.
- Removing all the ampicillin-treated replicates for this analysis.
...
# Load data
```{r}
control_gene_index = as.integer(scan("./non_regulated_gene_index_for_R.txt", what = "integer", sep = ","))

control_gene_index[1:10]
```
Sample names refer to condition:
T0 is without ampicillin treatment
T1 is after ampicillin treatment
OD is from a pilot RNA-seq run, not ampicillin treatment
L01: 0.1 mg/ml lactose in Davis minimal medium
L25: 2.5 mg/ml lactose in Davis minimal medium
L50: 50 mg/ml lactose in Davis minimal medium
```{r}
# load the data 
# round to integer values
countfile = './counts_no_amp.csv'
...
```{r}
# The last two genes in the countsdata object are not part of the transcriptome fastq.
for (i in range(dim(countsdata)[1] - 2)){
...
There were repeated names after changing the "OD" prefix to "T0". This is not desirable. 
...
sys_naming = function(dataset, condition) {
  ...
  # This function takes in a dataset, which should be an R dataframe, where each row is a variable, and each column is a sample name. The function will take in both the data frame and the condition names, as an indicator to return a systematically renamed dataframe. 
  # input:
  # dataset - the count dataframe
  # condition - a named vector for conditions, where the vector are the new patterns, and the name of the vector are the patterns that occurred in the column names
  # output:
...
```{r}
# use the function sys_naming to change the dataframe column names. 
conditions = c("pre_lac01_", "pre_lac25_", "pre_lac50_", "pst_lac01_", "pst_lac50_", "pre_lac01_", "pre_lac25_", "pre_lac50_")
names(conditions) = c("T0.L01", "T0.L25", "T0.L50", "T1.L01", "T1.L50", "OD.L01", "OD.L25", "OD.L50")
...
Note the design formula, a GLM for addition of all conditions. Other designs could be considered.
...
myvolcano = function(result_matrix, result_condition, desfolder, refhere, p_bar = 0.05){
...
resultcsv = function(result_matrix, result_condition, desfolder, refhere, p_bar = 0.05, getall=0){
  ...
  # If getall = 0, then the .csv records only significant genes
  # If getall = 1, then the .csv records the entire gene record
...
```{r}
deseqloop = function(dataobject, reflist, volcanodir, csvdir){
  dds_de_list = list()
  for (i in c(1:length(reflist))) {
    # factor function is available for targeted comparisions
    print(reflist[i])
    ...
    # print size factor
    size_factor = dds_de@colData@listData$sizeFactor    # the size factor for each replicate
    print(size_factor)
    DESeq2_norm = counts(dds_de, normalized=TRUE)    # obtain the normalized counts for each replicate
    ...
    for (j in c(2:length(renames))){
      ...
      # create volcano plot
      myvolcano(res, renames[j], volcanodir, reflist[i], p_bar = 0.05)
      # create csv files
...
if (!file.exists(vofolder)) {
 dir.create(file.path(vofolder))
}

csvfolder = "./DE-CSVs"
if (!file.exists(csvfolder)) {
 dir.create(file.path(csvfolder))
}
